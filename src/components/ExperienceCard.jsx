    ...
    return ( 
        ...
        <Fade left duration={1000} distance="40px">
            <Card style={{position: "relative", flexBasis: "auto", top: "100%", marginBottom: "2rem"}} className="shadow-lg--hover shadow border-0 text-center rounded">
                <CardHeader style={{background: data.color === undefined ? rgb(colorArrays)  : colorArrays}} >
                    <h5 className="text-white">{data.company}</h5>
                </CardHeader>
                <CardBody className="py-5">
                    <a href={data.link} target="_blank" rel="noopener noreferrer">
                        <img ref={imgRef} className=" bg-white rounded-circle mb-3 img-center img-fluid shadow-lg " src={data.companylogo} style={{ width: "100px", height: "100px" }} onLoad={() => getColorArrays()} alt=""/>
                    </a>
                    
                    <CardTitle tag="h5">{data.role}</CardTitle>
                    <CardSubtitle className="text-black" style={{paddingBottom: 15}}>{data.date}</CardSubtitle>
                    {moreInfo ? null : <Button onClick={() => changeInfo(!moreInfo)}>More Info</Button>}
